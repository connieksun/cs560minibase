...
int HeapFile::getRecCnt()
{
   ...
   while(dirPageId != INVALID_PAGE) {
     MINIBASE_BM->pinPage(dirPageId, (Page *&) dirPage, 0);
...
Status HeapFile::insertRecord(char *recPtr, int recLen, RID& outRid)
{
	...
	status = MINIBASE_BM->pinPage(dirPageId, (Page *&) dirPage, 0);
	...
	while (status != DONE) {
		...
		status = MINIBASE_BM->unpinPage(dataPageId, false, fileName); // unpin current data page
		...
		if (status == DONE) { // no more data page info records
			...
			status = MINIBASE_BM->unpinPage(dirPageId, false, fileName);
			...
			status = MINIBASE_BM->pinPage(dirPageId, (Page *&) dirPage, 0); // pin new page
    ...
    if (status == DONE || dataPageId == INVALID_PAGE) { // no data page with enough space
    	...
    	MINIBASE_BM->pinPage(dataPageId, (Page *&) dataPage, 0);
    	...
    	if ((int)dirPage->available_space() < (int)sizeof(DataPageInfo)) { // make another dir page
    		MINIBASE_BM->unpinPage(dirPageId, false, fileName); // unpin old directory page;
    		PageId newDirPageId;
    		HFPage* newDirPage;
    		...
    		MINIBASE_BM->unpinPage(newDirPageId, true, fileName); // unpin
...
Status HeapFile::deleteFile()
{
	...
	while (dirPageId != INVALID_PAGE) {
		...
		status = MINIBASE_BM->unpinPage(dirPageId, 0, fileName);
...
Status HeapFile::newDataPage(DataPageInfo *dpinfop)
{
	...
	MINIBASE_BM->unpinPage(pageId, true, fileName);
    return OK;
}
                    ...
                    RID &rpDataPageRid)
{
	...
	while (rpDirPageId != INVALID_PAGE){
		status = MINIBASE_BM->pinPage(rpDirPageId, (Page *&) rpdirpage, 0);
		...
		while (status != DONE) {
			...
			status = MINIBASE_BM->pinPage(rpDataPageId, (Page *&) rpdatapage, 0);
			status = rpdatapage->returnRecord(rid, recPtr, recLen);
			if (status == OK) {
				return OK;
			}
			status = MINIBASE_BM->unpinPage(rpDataPageId, false, fileName);
			status = rpdirpage->nextRecord(dataPageRid, nextRid);
			dataPageRid = nextRid;
		}
		status = MINIBASE_BM->unpinPage(rpDirPageId, false, fileName);
