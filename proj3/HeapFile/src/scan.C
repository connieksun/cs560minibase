...
Status Scan::reset()
{
	...
	if (dirPageId != INVALID_PAGE) {
		status = MINIBASE_BM->unpinPage(dirPageId, false, NULL);
		dirPageId = INVALID_PAGE;
	}
	if (dataPageId != INVALID_PAGE) {
		status = MINIBASE_BM->unpinPage(dataPageId, false, NULL);
		dataPageId = INVALID_PAGE;
	}
	dataPage = NULL;
	dirPage = NULL;
...
Status Scan::nextDataPage(){
	...
	status = MINIBASE_BM->unpinPage(dataPageId, false, NULL); // unpin current data page
	dataPageRid = nextRid;
	dirPage->returnRecord(dataPageRid, dpInfoRecPtr, dpInfoRecLen);
	dpInfoRecord = (DataPageInfo*) dpInfoRecPtr;
...
Status Scan::nextDirPage() {
	Status status = OK;
	status = MINIBASE_BM->unpinPage(dirPageId, false, NULL); // unpin current directory page
